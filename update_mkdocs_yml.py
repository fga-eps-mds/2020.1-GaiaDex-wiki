...
for filepath in glob('docs/**/*.md', recursive=True):
        ...
        for k2, v2 in d['docs'][k1].items():
            if type(v2) == dict:
                ...
                if 'Planejamento da Sprint' in sec_artifact_name or 'Resultados da Sprint' in sec_artifact_name:
                    sec_artifact_name = ' '.join(
                        sec_artifact_name.split(' ')[-2:])
                result['- ' + artifact_name]['- ' + sec_artifact_name] = {}
                for k3, v3 in d['docs'][k1][k2].items():
                    if k3 != 'name':
                        result['- ' + artifact_name][
                            '- ' + sec_artifact_name]['- ' + k3] = v3
            elif k2 != 'name':
                result['- ' + artifact_name]['- ' + k2] = v2

text_result = yaml.dump(result, allow_unicode=True,
                        default_flow_style=False, indent=4).replace('\'', '')
